...
class MySQLAdapter(DatabaseAdapter):
    ...
    def diagnose_connection(self) -> dict:
        ...
        masked_url = f"mysql://{self.user}:***@{self.host}:{self.port}/{self.default_database}"
        ...
        try:
            ...
            if "can't connect" in msg.lower() or "connection refused" in msg.lower():
                hints.append(f"Connection refused — is MySQL running on {self.host}:{self.port}?")
        ...
        try:
            ...
            result["server_version"] = cur.fetchone()["VERSION()"]
            cur.close()
